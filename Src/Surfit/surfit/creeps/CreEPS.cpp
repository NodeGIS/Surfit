		...
		if( file != NULL )
		{
			...
			switch( m_FillingPattern ) {
				...
				case HEXDOT :
					...
					BoxWidth  = m_ff1;
					BoxHeight = (float)sqrt(float(3))*m_ff1;
				...
				case STRIPE :
					...
					if( m_ff3 == 0 ) {
						...
						float diff = m_ff3 - float(3.14159265358979323846) / float(2);
						if( diff < 0 )  diff = -diff;
						// vertical lines
						if( diff < 0.0087266 ) {
						...
						// regular angle
							...
							BoxWidth  = float((m_ff1 + m_ff2) / sin(m_ff3));
							BoxHeight = float((m_ff1 + m_ff2) / cos(m_ff3));
							if( BoxHeight > 0 ) {
								startFillingBox( file, BoxWidth, BoxHeight );
								fprintf( file, "x %g sw "
								               "0 0 m %g %g l s "
                                    ...
                                    const CAt& A )
{
	...
	// -> rad
	m_ff3 *= (float(3.14159265358979323846) / float(180));
}
...
const float CreEPS::m_INCH2MM = float(72.0/25.4); // = 2.834645669291...
...
bool CreEPS::embedEPS( const char* const filename ) {
	...
	applyScaling( float(1.0/m_INCH2MM) );
